...
andM x y = x >>= \xm -> if xm then y else pure False
-- andM x y = x >>= \xm -> y >>= \ym -> pure (xm && ym)
-- andM x y = do
--   xm <- x
--   ym <- y
--   pure (xm && ym)
...
reverseTree (Node xs x ys) = Node (reverseTree ys) x (reverseTree xs)

treeToList :: Tree a -> [a]
treeToList Leaf = []
treeToList (Node xs x ys) = concat [treeToList xs, [x], treeToList ys]
